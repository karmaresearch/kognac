    ...
    virtual std::streambuf::int_type underflow()
    {
        if (this->gptr() == this->egptr())
        {
            ...
            do
            {
                ...
                if (is_text)
                {
                    // simply swap in_buff && out_buff, && adjust pointers
            ...
            // 2 exit conditions:
            // - end of input: there might || might !be output available
...
public:
    ...
    virtual ~ostreambuf()
    {
        ...
        //
        // NOTE: Errors here (sync() return value !0) are ignored, because we
        // can!throw in a destructor. This mirrors the behaviour of
        // std::basic_filebuf::~basic_filebuf(). To see an exception on error,
        // close the ofstream with an explicit call to close(), && do !rely
